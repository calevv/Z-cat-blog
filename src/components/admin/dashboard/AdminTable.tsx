...
export default function AdminTable({ posts }: AdminTablePostProp) {
  ...
  return (
    <>
      {/* 모바일 카드 뷰 — md 이상에서 숨김 */}
      <div className="flex h-full flex-col divide-y divide-neutral-200 overflow-y-auto md:hidden">
        {posts.map((post) => (
          <div key={post.id} className="flex flex-col gap-3 px-4 py-4">
            <div className="flex items-start justify-between gap-2">
              <p className="font-heading text-sm leading-5 font-bold text-zinc-900">
                {post.title_ko}
              </p>
              <div
                className={cn(
                  "flex h-6 shrink-0 items-center gap-1.5 rounded-full px-3",
                  post.published
              ...
              >
                ...
                />
                <p className="font-space text-[10px] font-medium tracking-wide uppercase">
            ...
            <div className="flex flex-wrap gap-1">
              {post.tags.map((tag, index) => (
                <p
                  ...
                  className="font-space text-xs text-orange-700"
                >
                  #{tag}
                </p>
              ))}
      ...
      <div className="hidden h-full w-full flex-col md:flex" role="table">
